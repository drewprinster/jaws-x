'''
Note: This script is an update to the active learning experiments in 'run_JAWS-X_active.py', where this updated version was not used in the ICML 2023 
version of the 'JAWS-X' paper. The original active learning experiments in 'JAWS-X' were only on one 'toy' dataset, and not extremely rigorous; 
so, this updated script aims to provide more rigorous active learning experiments in two main ways:

    1. Querying new points based on based on *posterior model variance* (uncertainty in the underlying model)
       rather than *predicted* variance (uncertainty in the prediction of the label). This ensures that the query
       strategy is a more realistic max-entropy uncertainty sampling strategy, where points are selected based on
       epistemic uncertainty, rather than selecting points that are high aleatoric uncertainty (but possibly low 
       epistemic uncertainty). 
       
    2. Querying new points based on an *exponential tilt* of (epistemic) uncertainty, rather than proportional to 
       the (model) variance. This allows the query strategy to be more realistically aggressive in querying 
       points with high epistemic uncertainty.
...
## Function added 2024-06-20 for sampling based on *posterior model variance* (uncertainty in the underlying model)
## rather than *predicted* variance (uncertainty in the prediction of the label)
...
#     parser.add_argument('--fitness_str', type=str, default='red', help='Red or blue fluorescence experiments.')
    ...
    parser.add_argument('--lmbda', type=float, default=1.0, help='Shift magnitude or aggressiveness of querying')
...
#     ytest_s, predtest_s = np.zeros([n_seed, n1]), np.zeros([n_seed, n1])
    ...
    for seed in range(seed_initial, seed_initial + n_seed):
        ...
        for step in range(n_steps):
            ...
            y_preds_pool, std_preds_pool_ = gpr.predict(Xpool, return_std=True) ## Predictions on pool
            
            std_preds_pool = get_f_std(std_preds_pool_, gpr) ## Line added 20240620
            
            var_preds_pool = std_preds_pool**2
            var_preds_pool_minmax_normed = (var_preds_pool) / (max(var_preds_pool) - min(var_preds_pool))
            exp_var_preds_pool = np.exp(var_preds_pool_minmax_normed * lmbda)
            exp_var_preds_pool_norm = exp_var_preds_pool / np.sum(exp_var_preds_pool)
            query_ann_inds = list(np.random.choice(pool_inds, n_queries_ann, replace=False, p=exp_var_preds_pool_norm)) 

            
            ## Query points for coverage evaluation from validation set by sampling in proportion to variance
            y_preds_val, std_preds_val_ = gpr.predict(Xval, return_std=True)
            
            std_preds_val = get_f_std(std_preds_val_, gpr) ## Line added 20240620
            
            var_preds_val = std_preds_val**2
            var_preds_val_minmax_normed = (var_preds_val) / (max(var_preds_val) - min(var_preds_val))
            exp_var_preds_val = np.exp(var_preds_val_minmax_normed * lmbda)
            exp_var_preds_val_norm = exp_var_preds_val / np.sum(exp_var_preds_val)
            query_cov_inds = list(np.random.choice(val_inds, n_queries_cov, replace=False, p=exp_var_preds_val_norm)) 
            ...
            ## Query point(s) for annotation from pool based on max predicted variance (max entropy)
            y_preds_pool_split, std_preds_pool_split_ = gpr_split.predict(Xpool_split, return_std=True) ## Predictions on pool

            std_preds_pool_split = get_f_std(std_preds_pool_split_, gpr_split) ## Line added 20240620
            
            var_preds_pool_split = std_preds_pool_split**2
            var_preds_pool_minmax_normed_split = (var_preds_pool_split) / (max(var_preds_pool_split) - min(var_preds_pool_split))
            exp_var_preds_pool_split = np.exp(var_preds_pool_minmax_normed_split * lmbda)
            exp_var_preds_pool_split_norm = exp_var_preds_pool_split / np.sum(exp_var_preds_pool_split)
            query_ann_inds_split = list(np.random.choice(pool_inds_split, n_queries_ann, replace=False, p=exp_var_preds_pool_split_norm)) 

            
            ## Query points for coverage evaluation from validation set by sampling in proportion to variance
            y_preds_val_split, std_preds_val_split_ = gpr_split.predict(Xval, return_std=True)
            
            std_preds_val_split = get_f_std(std_preds_val_split_, gpr_split) ## Line added 20240620
            
            var_preds_val_split = std_preds_val_split**2
            var_preds_val_minmax_normed_split = (var_preds_val_split) / (max(var_preds_val_split) - min(var_preds_val_split))
            exp_var_preds_val_split = np.exp(var_preds_val_minmax_normed_split * lmbda)
            exp_var_preds_val_split_norm = exp_var_preds_val_split / np.sum(exp_var_preds_val_split)
            query_cov_inds_split = list(np.random.choice(val_inds, n_queries_cov, replace=False, p=exp_var_preds_val_split_norm)) 
...
#                 print(n1, len(coverage_all), len(muh_test_all))
    ...
    results_by_seed.to_csv(os.getcwd().removesuffix('bash_scripts') + 'results/'+ str(date.today()) + '_UpdatedAL_' + dataset + '_' + muh + '_itrain' + str(n_train_initial) + '_steps' + str(n_steps) + '_nseed' + str(n_seed) + '_iseed' + str(seed_initial) + '_lmbda' + str(lmbda) + '_PIs_results_BySeed_v2.csv',index=False)
    
    results_all.to_csv(os.getcwd().removesuffix('bash_scripts') + 'results/'+ str(date.today()) + '_UpdatedAL_' + dataset + '_' + muh + '_itrain' + str(n_train_initial) + '_steps' + str(n_steps) + '_nseed' + str(n_seed) + '_iseed' + str(seed_initial) + '_lmbda' + str(lmbda) + '_PIs_results_ALL_v2.csv',index=False)
