...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_hidden', type=int, default=100, help='Number of hidden nodes in IF bayesnn.')
...
#                 print("ycal_split", ycal_split)
                ...
                PIs = jaw_fcs_nn.compute_PIs_IFs(Xtrain_nxp, ytrain_n, Xtest_n1xp, ytest_n1, pred_train_test, Xtrain_split, Xcal_split, ytrain_split, ycal_split, Xtest_n1xp_split, ytest_n1_split, pred_cal_test_split, params, params_split, lmbda, alpha=alpha, K_vals = K_vals, L2_lambda = L2_lambda)
...
#                         print(PIs[method]['lower'][0:10], ytest_n1_split[0:10], PIs[method]['upper'][0:10])
                ...
                if ((seed + 1) == 5 or (seed + 1) == 10):
                    results_by_seed.to_csv(str(date.today()) + '_nhid' +  str(num_hidden) + '_' + fitness_str + '_' + muh + '_ntrain' +  str(n_train) + '_lmbda' + str(lmbda) + '_seed' + str(seed + 1) + '_' + str(L2_lambda) + 'L2_PIs_results_BySeed_IFs.csv',index=False)

results_by_seed.to_csv(str(date.today()) + '_nhid' +  str(num_hidden) + '_' + fitness_str + '_' + muh + '_ntrain' +  str(n_train) + '_lmbda' + str(lmbda) + '_seed' + str(seed + 1) + '_' + str(L2_lambda) + 'L2_PIs_results_BySeed_IFs.csv',index=False)

# if (n_trains[0] == 192):
results_all.to_csv(str(date.today()) + '_nhid' +  str(num_hidden) + '_' + fitness_str + '_' + muh + '_ntrain' +  str(n_train) + '_lmbda' + str(lmbda) + '_seed' + str(seed + 1) + '_' + str(L2_lambda) + 'L2_PIs_results_IFs_ALL.csv',index=False)
